...
mg_I(N) struct wav_basis_norms_static {
  stack_array<f64, N> ScalNorms;
  stack_array<f64, N> WaveNorms;
};
mg_I(N) wav_basis_norms_static<N> GetCdf53NormsFast();
...
mg_T(t) void FLiftCdf53X(const grid& Grid, const v3i& M3, lift_option Opt, volume* Vol);
mg_T(t) void FLiftCdf53Y(const grid& Grid, const v3i& M3, lift_option Opt, volume* Vol);
mg_T(t) void FLiftCdf53Z(const grid& Grid, const v3i& M3, lift_option Opt, volume* Vol);
/* The inverse lifting functions can be used to extrapolate the volume */
mg_T(t) void ILiftCdf53X(const grid& Grid, const v3i& M3, lift_option Opt, volume* Vol);
mg_T(t) void ILiftCdf53Y(const grid& Grid, const v3i& M3, lift_option Opt, volume* Vol);
mg_T(t) void ILiftCdf53Z(const grid& Grid, const v3i& M3, lift_option Opt, volume* Vol);
    ...
    for (int y = P.y; y < P.y + S.y * D.y; y += S.y) {\
      ...
      if (Opt != lift_option::NoUpdate) {\
        ...
        if (!Ext) { /* no extrapolation, inverse update at the last odd position */\
          ...
          t A = F[mg_Row##x(x0, yy, zz, N)];\
          t B = F[mg_Row##x(x2, yy, zz, N)];\
          F[mg_Row##x(x1, yy, zz, N)] = (A + B) / 2;\
        }\
      }\
