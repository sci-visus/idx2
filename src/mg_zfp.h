...
mg_T(t) void ForwardZfp(t* P, int D);
mg_T(t) void InverseZfp(t* P);
mg_T(t) void InverseZfp(t* P, int D);
...
mg_TT(t, u) void ForwardShuffle(t* IBlock, u* UBlock, int D);
mg_TT(t, u) void InverseShuffle(u* UBlock, t* IBlock, int D);
...
mg_T(t) void Encode(t* Block, int NVals, int B, i8& N, bitstream* Bs);
mg_T(t) void Decode(t* Block, int NVals, int B, i8& N, bitstream* Bs);
...
mg_TTi(t, u) void
ForwardShuffle(t* mg_Restrict IBlock, u* mg_Restrict UBlock, int D) {
  auto Mask = traits<u>::NBinaryMask;
  switch (D) {
...
mg_TTi(t, u) void
InverseShuffle(u* mg_Restrict UBlock, t* mg_Restrict IBlock, int D) {
  auto Mask = traits<u>::NBinaryMask;
  switch (D) {
...
mg_T(t) void
Encode(t* mg_Restrict Block, int NVals, int B, /*i64 S, */i8& N, bitstream* mg_Restrict BsIn) {
  static_assert(is_unsigned<t>::Value);
  mg_Assert(NVals <= 64); // e.g. 4x4x4, 4x4, 8x8
  bitstream Bs = *BsIn;
  u64 X = 0;
...
mg_Inline void
DecodeTest(u64* mg_Restrict Block, int NVals, i8& N, bitstream* mg_Restrict BsIn) {
  mg_Assert(NVals <= 64); // e.g. 4x4x4, 4x4, 8x8
  bitstream Bs = *BsIn;
  i8 P = N;
  u64 X = P > 0 ? ReadLong(&Bs, P) : 0;
...
// NOTE: This is the one being used
mg_T(t) void
Decode(t* mg_Restrict Block, int NVals, int B, /*i64 S, */i8& N, bitstream* mg_Restrict BsIn) {
  static_assert(is_unsigned<t>::Value);
  mg_Assert(NVals <= 64); // e.g. 4x4x4, 4x4, 8x8
  bitstream Bs = *BsIn;
//  i8 P = (i8)Min((i64)N, S - BitSize(Bs));
