...
mg_I(N) wav_basis_norms_static<N>
GetCdf53NormsFast() {
...
ExtrapolateCdf53(const v3i& Dims3, u64 TransformOrder, volume* Vol) {
  ...
  // printf("M3 = " mg_PrStrV3i "\n", mg_PrV3i(M3));
  mg_Assert(IsPow2(M3.X) && IsPow2(M3.Y) && IsPow2(M3.Z));
...
ForwardCdf53(const v3i& Dims3, const v3i& M3, int Iter, int NLevels, u64 TformOrder, volume* Vol, bool Normalize) {
  ...
  while (Level < NLevels) {
    ...
    if (D == 3) { // next level
      ...
      #define Body(type)\
      switch (D) {\
        case 0: mg_Assert(Dims3.X > 1); FLiftCdf53X<type>(G, M3, lift_option::Normal, Vol); break;\
        case 1: mg_Assert(Dims3.Y > 1); FLiftCdf53Y<type>(G, M3, lift_option::Normal, Vol); break;\
        case 2: mg_Assert(Dims3.Z > 1); FLiftCdf53Z<type>(G, M3, lift_option::Normal, Vol); break;\
        default: mg_Assert(false); break;\
      };
      mg_DispatchOnType(Vol->Type);
      #undef Body
  ...
  if (NLevels > 0 && Normalize) {
    mg_Assert(IsFloatingPoint(Vol->Type));
    mg_RAII(array<subband>, Subbands, BuildSubbands(Dims3, NLevels, TformOrder, &Subbands));
  ...
  while (Level < NLevels) {
    ...
    if (D == 3) { // next level
      ...
      for (int I = 0; I < Sz; ++I) {
        ...
        v3<i8> NextLowHigh3 = Queue[0].LowHigh3; mg_Assert(NextLowHigh3[D] == 0); NextLowHigh3[D] = 1;
...
BuildLevelGrids(const v3i& N3, int NLevels, u64 TransformOrder, array<grid>* LevelGrids) {
  ...
  mg_Assert(Size(*LevelGrids) == NLevels + 1);
...
Extrapolate3D(v3i D3, volume* Vol) {
  if constexpr (is_same_type<t, f32>::Value)
    mg_Assert(Vol->Type == dtype::float32);
  if constexpr (is_same_type<t, f64>::Value)
    mg_Assert(Vol->Type == dtype::float64);
  v3i N3 = Dims(*Vol);
  mg_Assert(N3 > 1);
  mg_Assert(D3 <= N3);
  mg_Assert(N3.X == N3.Y && N3.Y == N3.Z);
  mg_Assert(IsPow2(N3.X - 1));
  v3i R3 = D3 + IsEven(D3);
  v3i S3(1); // Strides
