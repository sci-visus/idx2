...
struct hash_table {
  ...
  struct iterator {
    ...
    bool operator==(const iterator& Other) const;
    operator bool() const;
    mg_Inline v& operator*() { return *Val; }
  };

  mg_Inline v& operator[](const k& Key) {
  ...
  mg_Inline operator bool() const { return Keys != nullptr; }
};
...
mg_TTi(k, v) bool hash_table<k, v>::iterator::
...
mg_TTi(k, v) hash_table<k, v>::iterator::
operator bool() const {
  return Ht->Stats[Idx] == Occupied;
}

mg_TTi(k, v) typename hash_table<k, v>::iterator
...
mg_TT(k, v) typename hash_table<k, v>::iterator
...
End(const hash_table<k, v>& Ht) {
  i64 C = Capacity(Ht);
  mg_Assert((Ht.Stats[C] == hash_table<k, v>::Occupied));
  return IterAt(Ht, C);
}

mg_TTi(k, v) typename hash_table<k, v>::iterator& hash_table<k, v>::iterator::
operator++() {
  do {
    ++Idx;
...
Lookup(hash_table<k, v>* Ht, const k& Key) {
  ...
  if (Found) { mg_Assert(*Result.Key == Key); }
  return Result;
}
...
Delete(hash_table<k, v>* Ht, const k& Key) {
  ...
  if (Found) { mg_Assert(Ht->Keys[H] == Key); }
  mg_Assert(Found);
  return End(*Ht);
}
