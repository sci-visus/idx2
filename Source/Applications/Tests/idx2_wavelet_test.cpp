...
//      FLiftCdf53Z<f64>(grid(v3i(0), v3i(R3.X, R3.Y, D3.Z), S3), M3, lift_option::Normal, &Vol);
    ...
    printf("S3 = %d %d %d\n", S3.X, S3.Y, S3.Z);
    // TODO: need to reset S3 and perhaps other variables between the I loops
    /* Produce one column per loop */
    for (int I = 0; I < M3.X; ++I) {
      Vol = 0.0;
      if ((I % 2) == 0) {
        Vol.At<f64>(v3i(I, 0, 0)) = 1;
      for (int L = NLevels - 1; L >= 0; --L) {
        S3 = S3 / 2;
        printf("L = %d S3 = %d %d %d\n", L, S3.X, S3.Y, S3.Z);
  //      ILiftCdf53Z<f64>(grid(v3i(0), Dims3Stack[I].Z, S3), M3, lift_option::Normal, &Vol);
  //      ILiftCdf53Y<f64>(grid(v3i(0), Dims3Stack[I].Y, S3), M3, lift_option::Normal, &Vol);
        ILiftCdf53X<f64>(grid(v3i(0), Dims3Stack[L].X, S3), M3, lift_option::Normal, &Vol);
      }
      for (int J = 0; J < M3.X; ++J)
        fprintf(Fp, "%f ", Vol.At<f64>(v3i(J, 0, 0)));
      fprintf(Fp, "\n");
    }
  }
//    for (int I = 0; I < M3.X; ++I) {
...
//          ILiftCdf53X<f64>(grid(v3i(0), v3i(D3.X, D3.Y, D3.Z), S3), M3, lift_option::Normal, &Vol);
//  //        FLiftCdf53Y<i64>(grid(v3i(0), v3i(R3.X, D3.Y, D3.Z), S3), M3, lift_option::Normal, &Vol);
//  //        FLiftCdf53Z<i64>(grid(v3i(0), v3i(R3.X, R3.Y, D3.Z), S3), M3, lift_option::Normal, &Vol);
//          D3 = (R3 + 1) / 2;
//          R3 = D3 + IsEven(D3);
//          S3 = S3 * 2;
//        }
