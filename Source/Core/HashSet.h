...
struct hash_set {
  ...
  struct iterator {
    ...
    bool operator==(const iterator& Other) const;
    operator bool() const;
    idx2_Inline k& operator*() { return *Key; }
  };
...
idx2_T(k)
u64 HeapSize(const hash_set<k>& HashSet) {
...
idx2_Ti(k) bool hash_set<k>::iterator::
operator!=(const hash_set<k>::iterator& Other) const {
...
idx2_Ti(k) hash_set<k>::iterator::
operator bool() const {
  return Hs->Stats[Idx] == Occupied;
}
/* Create an iterator at position H */
idx2_Ti(k) typename hash_set<k>::iterator
IterAt(const hash_set<k>& Ht, i64 H) {
  return typename hash_set<k>::iterator
    { &(Ht.Keys[H]), &(const_cast<hash_set<k>&>(Ht)), H };
}

/* Begin iterator */
idx2_T(k) typename hash_set<k>::iterator
...
/* Advance iterator */
idx2_Ti(k) typename hash_set<k>::iterator& hash_set<k>::iterator::
operator++() {
  do {
    ++Idx;
...
idx2_Ti(k) u64
Index(hash_set<k>* Hs, u64 Key) { // Fibonacci hashing
...
Lookup(hash_set<k>* Hs, const k& Key) {
  ...
  i64 Start = H;
  bool Found = false;
  while (Hs->Stats[H] != hash_set<k>::Empty) { // either Occupied or Tombstone
    if (Hs->Keys[H] == Key) { Found = true; break; }
    ++H;
    if ((H &= Capacity(*Hs) - 1) == Start) break;
  }
  if (!Found) {
...
Delete(hash_set<k>* Hs, const k& Key) {
  ...
  if (Found) { idx2_Assert(Hs->Keys[H] == Key); }
  idx2_Assert(Found);
  return End(*Hs);
}
